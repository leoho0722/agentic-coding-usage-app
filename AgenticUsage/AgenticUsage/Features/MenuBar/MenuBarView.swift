...
extension MenuBarView {
    ...
    private var claudeNotDetectedContent: some View {
        VStack(alignment: .leading, spacing: 10) {
            if store.claude.isLoading {
                ...
                Text("Please sign in to Claude Code via terminal first.")
                    .font(.caption)
                    .foregroundStyle(.secondary)

                Button("Re-detect") {
                    store.send(.claude(.detectCredentials))
                }
                .buttonStyle(.borderedProminent)
                .controlSize(.small)
            }
            ...
            // 錯誤訊息提示（偵測失敗時顯示）
            if let error = store.claude.errorMessage {
                HStack(spacing: 6) {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .foregroundStyle(.yellow)
                    Text(error)
                    ...
                    Spacer()
                    Button {
                        store.send(.claude(.dismissError))
                    } label: {
                        Image(systemName: "xmark.circle")
                    }
                    .buttonStyle(.borderless)
    ...
    private var claudeConnectedContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            if store.claude.isLoading {
                ...
                .padding(12)
            } else if let summary = store.claude.usageSummary {
            ...
            HStack {
                Button {
                    store.send(.claude(.fetchUsage))
                } label: {
                    Label("Refresh", systemImage: "arrow.clockwise")
                        .font(.caption)
                }
                .buttonStyle(.borderless)
                .disabled(store.claude.isLoading)
    ...
    @ViewBuilder
    private func claudeProgressRow(label: LocalizedStringKey, utilization: Int, countdown: String?) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                Text(label)
                    .font(.caption)
...
extension MenuBarView {
    ...
    private var codexNotDetectedContent: some View {
        VStack(alignment: .leading, spacing: 10) {
            if store.codex.isLoading {
                ...
                Text("Please sign in to Codex via terminal first.")
                    .font(.caption)
                    .foregroundStyle(.secondary)

                Button("Re-detect") {
                    store.send(.codex(.detectCredentials))
                }
                .buttonStyle(.borderedProminent)
                .controlSize(.small)
            }
            ...
            // 錯誤訊息提示（偵測失敗時顯示）
            if let error = store.codex.errorMessage {
                HStack(spacing: 6) {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .foregroundStyle(.yellow)
                    Text(error)
    ...
    private var codexConnectedContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            if store.codex.isLoading {
                ...
                .padding(12)
            } else if let summary = store.codex.usageSummary {
            ...
            HStack {
                Button {
                    store.send(.codex(.fetchUsage))
                } label: {
                    Label("Refresh", systemImage: "arrow.clockwise")
                        .font(.caption)
                }
                .buttonStyle(.borderless)
                .disabled(store.codex.isLoading)
    ...
    ///   - countdown: 重設倒數文字，若無則隱藏
    @ViewBuilder
    private func codexProgressRow(label: LocalizedStringKey, usedPercent: Int, countdown: String?) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                Text(label)
                    .font(.caption)
...
extension MenuBarView {
    ...
    private var antigravityNotDetectedContent: some View {
        VStack(alignment: .leading, spacing: 10) {
            if store.antigravity.isLoading {
                HStack {
                    Spacer()
                    ProgressView()
                        .controlSize(.small)
                ...
                Text("Please sign in to Google Antigravity IDE first.")
                    .font(.caption)
                    .foregroundStyle(.secondary)

                Button("Re-detect") {
                    store.send(.antigravity(.detectCredentials))
                }
                .buttonStyle(.borderedProminent)
                .controlSize(.small)
            }
            ...
            // 錯誤訊息提示（偵測失敗時顯示）
            if let error = store.antigravity.errorMessage {
                HStack(spacing: 6) {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .foregroundStyle(.yellow)
                    Text(error)
                    ...
                    Spacer()
                    Button {
                        store.send(.antigravity(.dismissError))
                    } label: {
                        Image(systemName: "xmark.circle")
                    }
                    .buttonStyle(.borderless)
    ...
    private var antigravityConnectedContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            if store.antigravity.isLoading {
                ...
                .padding(12)
            } else if let summary = store.antigravity.usageSummary {
            ...
            HStack {
                Button {
                    store.send(.antigravity(.fetchUsage))
                } label: {
                    Label("Refresh", systemImage: "arrow.clockwise")
                        .font(.caption)
                }
                .buttonStyle(.borderless)
                .disabled(store.antigravity.isLoading)
